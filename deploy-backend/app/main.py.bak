...
app.add_middleware(
    ...
    allow_methods=["*"],  # Allows all methods, including the OPTIONS knock!
    allow_headers=["*"],
)
...
def register_user(user: UserCreate, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(db_user)
    return {"status": "success", "message": f"{user.role.capitalize()} account created successfully!"}
...
def login_user(login_data: UserLogin, db: Session = Depends(get_db)):
    db_user = db.query(models.User).filter(models.User.phone_number == login_data.phone).first()
    if not db_user:
        raise HTTPException(status_code=400, detail="Invalid phone number or password.")

    valid_password = bcrypt.checkpw(login_data.password.encode('utf-8'), db_user.password_hash.encode('utf-8'))
...
def add_produce(produce: ProduceCreate, db: Session = Depends(get_db)):
    farmer = db.query(models.User).filter(models.User.phone_number == produce.farmer_phone).first()
    ...
    return {"status": "success", "message": f"{produce.crop_type} listed successfully!"}


# --- FETCH ALL PRODUCE ---
@app.get("/api/produce")
def get_all_produce(db: Session = Depends(get_db)):
    results = db.query(models.Produce, models.User.full_name, models.User.phone_number)\
...
def create_order(order_data: OrderCreate, db: Session = Depends(get_db)):
    ...
    for item in order_data.items:
        ...
        # --- STOCK GUARD ---
        # Calculate how many kg this order represents
        if produce.price <= 0:
            raise HTTPException(
                status_code=400,
...
def update_order_status(
    ...
    produce = db.query(models.Produce).filter(
        models.Produce.produce_id == order.produce_id
    ).first()
    farmer = db.query(models.User).filter(
        models.User.phone_number == update.farmer_phone
    ).first()
    ...
    if update.status == "Accepted" and produce.price > 0:
        ...
        old_qty = produce.quantity
        produce.quantity = max(0.0, produce.quantity - qty_ordered)

        db.add(produce)  # The magic fix!

        print(f"\n[ORDER ACCEPTED] Order #{order_id} | Crop: {produce.crop_type} | "
              f"Stock: {old_qty} → {produce.quantity} kg (deducted {qty_ordered} kg)\n")
